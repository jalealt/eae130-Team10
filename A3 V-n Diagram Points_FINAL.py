...
S_ref = 529.37186  # ft^2, reference wing area
...
n_pos_limit = compute_positive_limit_loads(MTOW)
...
n_exceed_line = np.linspace(0, 2.53, 100)  
...
n_pos_value = compute_positive_limit_loads(MTOW)  # Recalculate actual value
Va = np.sqrt(abs(n_pos_value) / stall_coeff)
...
# Vb (Design for Max Gust Intensity)
#Vb = 580 #looking at graphical intersection at sea level gust MAX WEIGHT
Vb = 500 #looking at graphical intersection at sea level gust MAX WEIGHT
